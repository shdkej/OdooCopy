...
var FieldMany2ManyBinaryMultiFiles = AbstractManyField.extend(common.ReinitializeFieldMixin, {
    ...
    events: {
        ...
        'change .o_form_input_file2_visible': function(e) {
           var self = this;
	   var url = window.location.origin;
           var uploadURL = url + "/web/binary/upload_attachment";
           _.each(e.target.files, function(file){
            var querydata = new FormData();
            querydata.append('callback', 'oe_fileupload_temp2');
            querydata.append('ufile', file);
            querydata.append('model', 'purchase.order');
            ...
            $.ajax({
              ...
              data: querydata,
              success: function(id){
                var idtostring = id.replace(/'/g,"\"");
                //.replace(/u|c/g,"");
                var tojson = JSON.parse(idtostring);
                var intid = tojson.id;
                self.data[intid] = {
                  'id': intid,
                  'name': file.name,
                ...
                tojson.url = self.get_file_url(tojson);
                tojson.name = file.name;
                self.data[tojson.id] = tojson;
                var values = _.clone(self.get('value'));
                values.push(tojson.id);
...
var FieldMany2ManyBinaryMultiFiles2 = AbstractManyField.extend(common.ReinitializeFieldMixin, {
    template: "FieldBinaryFileUploader2",
    events: {
        'click .o_attach': function(e) {
            this.$('.o_form_input_file').click();
        },
        'change .o_form_input_file2': function(e) {
           var self = this;
	   var url = window.location.origin;
           var uploadURL = url + "/web/binary/upload_attachment";
           _.each(e.target.files, function(file){
            ...
            querydata.append('callback', 'oe_fileupload_temp2');
            querydata.append('ufile', file);
            querydata.append('model', 'gvm.purchase_product');
            querydata.append('id', '0');
            querydata.append('multi', 'true');
            $.ajax({
              url: uploadURL,
...
core.form_widget_registry
    ...
    .add('many2many_binary2', FieldMany2ManyBinaryMultiFiles2)
