...
class DataSet(http.Controller):
    ...
    def gvm_pre_payment_confirm(self):
      ...
      subject = "[GVM]"+ post_id + " 선입금 완료."

      self.gvm_send_mail(model_name, menu_id, action_id, post_id, receivers, subject)
    
    def gvm_send_mail(self, model_name, menu_id, action_id, post_id, receivers, subject):
...
class ExcelExport(ExportFormat, http.Controller):
	...
	part_style = xlwt.easyxf('align: wrap yes; font:bold on; pattern: pattern solid, fore_colour custom_colour')

        i = 1
	prev_issue = ''
        for row_index, row in enumerate(rows):
	    if issue_index:
	      if prev_issue != row[issue_index]:
	        prev_issue = row[issue_index]
                worksheet.write(row_index + i, 0, row[issue_index], part_style)
	        worksheet.merge(row_index + i, 0, 0, 10)
	        i = i+1
            for cell_index, cell_value in enumerate(row):
                ...
                worksheet.write(row_index + i, cell_index, cell_value, cell_style)
