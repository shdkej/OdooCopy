...
class PurchaseOrder(models.Model):
    ...
    def _compute_invoice(self):
        for order in self:
            ...
            invoices = self.env['account.invoice'].search([('purchase_id','=',order.id)])
...
#        'cancel': [('readonly', True)],
    ...
    product = fields.One2many('gvm.product','purchase',string='발주', index=True)
    ...
    purchase_product = fields.Many2many('gvm.product',string='발주')
    ...
    def create(self, vals):
        ...
        last_id = self.env['purchase.order'].search([('id','=',res.id)])
    ...
    def unlink(self):
        state = ['write','cancel']
        for order in self:
	    if order.purchase_product:
	      for pd in order.purchase_product:
	        pd.state = 'delete'
    ...
    def button_send_quotation(self):
        ...
        post = '견적요청서'
        #self.gvm_send_mail(send ,marketing, post)
    ...
    def button_confirm(self):
	    ...
	    for product in order.product:
	      product.write({'state':'purchase_complete'})
