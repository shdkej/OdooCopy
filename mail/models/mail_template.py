	    ...
	    _logger.warning(template)
            ...
            if template.report_template:
                for res_id in template_res_ids:
                    ...
                    att1 = self.env['purchase.order'].search([('id','=',res_id)])
                    if att1.attachment:
                     for att_pur in att1.attachment:
                      if att_pur:
                       attachments.append((att_pur.name, att_pur.datas))
		    if att1.purchase_product:
		      for purchase_product in att1.purchase_product:
		        if purchase_product.attachment:
			  for att_pur in purchase_product.attachment:
                            attachments.append((att_pur.name, att_pur.datas))
