...
class Project(models.Model):
    ...
    percent = fields.Float('진행도',compute='_compute_percent')
    is_finish = fields.Boolean('is_finish',store=True)
    site = fields.Char('site')
    sites = fields.Many2one('project.site', '사이트')
...
#    sign = fields.One2many('gvm.signcontent','project','sign')
    ...
    def on_change_issue(self):
      ...
      if self.issue_import:
       for iss in self.issue_import:
         newissue = issue.create({'name':iss.name,
                      'partner_id':iss.partner_id.id,
                      'email_from':iss.email_from,
                      'project_id':self.id,
                      'line_count':iss.line_count,
                      'department':iss.department,
                      })
         if iss.product:
           for pd in iss.product:
             newissue.write({'product':[
	          (0, 0, {'name': pd.name,
	 ...
	 if iss.attachment:
	   for att in iss.attachment:
	     newissue.write({'attachment':[(4,att.id)]})
         self.issue_import = ''
      return {}
...
class ProjectTimeline(models.Model):
    ...
    name = fields.Char('설명')
    color = fields.Integer(string='Color Index')
    date_from = fields.Date('시작일')
    date_to = fields.Date('종료일')
