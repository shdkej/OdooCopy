...
class ProjectIssue(models.Model):
    ...
    purchase_ids = fields.Many2many('project.task', string='Task')
    ...
    purchase_button = fields.Selection([('1','new'),('2','old')],default='1')
    ...
    def button_create_purchase(self):
        ...
        newPo = po.create({'project':self.project_id.id,
        ...
        for np in newProduct:
         newPo.write({'product':[(4,np.id)]})
         np.write({'purchase_order_new':True})

        task.create({'name': self.name,
                     'kanban_state':'normal',
                     'project_id':self.project_id.id
                    })
        self.write({'purchase_button':'2'})
        return {}
    @api.multi
    def button_create_purchase_new(self):
        po = self.env['purchase.order']
        newPo = po.create({'project':self.project_id.id,
                   'partner_id':self.partner_id.id,
                   'line_count':self.line_count,
                   'department':self.department,
                   'attachment':[(4,[a.id for a in self.attachment])],
                   'issue':self.id
                  })
        for pd in self.product:
         if not pd.purchase_order_new:
          newPo.write({'product':[(4,pd.id)]})
          pd.write({'purchase_order_new':True})
        return {}
    ...
    def create(self, vals):
        ...
        res = super(ProjectIssue, self.with_context(context)).create(vals)
