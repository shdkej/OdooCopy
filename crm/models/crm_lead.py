...
class Lead(FormatAddress, models.Model):
    ...
    history1 = fields.One2many('crm.lead.history1','crm','견적서')
    history2 = fields.One2many('crm.lead.history2','crm','제안서')
    history3 = fields.One2many('crm.lead.history3','crm','스케줄표')
    history4 = fields.One2many('crm.lead.history4','crm','P.O.')
    history5 = fields.One2many('crm.lead.history5','crm','수금현황')
    site = fields.Char('사이트')
    project_code = fields.Char('프로젝트 코드')
    world = fields.Boolean('해외 프로젝트')
    create_project = fields.Boolean('check')
    project = fields.One2many('project.project','crm','project')
    ...
    def button_create_project(self):
        ...
        for record in self:
           project.create({'name':record.name,'crm':record.id,'code':record.project_code,'site':record.site,'world':record.world,'type_ids':[(4,[24,25,60,61,62])]})
        self.write({'create_project':True})
        return {}
    @api.multi
...
class LeadHistory1(models.Model):
    _name = "crm.lead.history1"
    _description = 'Lead History1'

    name = fields.Char('Name', required=True, translate=True)
    active = fields.Boolean('Active', default=False)
    date_to = fields.Date('end')
    date_from = fields.Date('start')
    text = fields.Text('text')
    crm = fields.Many2one('crm.lead','crm')
    attachment = fields.Many2many('ir.attachment', domain="[('res_model','=','crm.lead')]", string='첨부파일')
class LeadHistory2(models.Model):
    _name = "crm.lead.history2"
    _description = 'Lead History2'
    ...
    crm = fields.Many2one('crm.lead','crm')
    attachment = fields.Many2many('ir.attachment', domain="[('res_model','=','crm.lead')]", string='첨부파일')
class LeadHistory3(models.Model):
    _name = "crm.lead.history3"
    _description = 'Lead History3'
    ...
    crm = fields.Many2one('crm.lead','crm')
    attachment = fields.Many2many('ir.attachment', domain="[('res_model','=','crm.lead')]", string='첨부파일')
class LeadHistory4(models.Model):
    _name = "crm.lead.history4"
    _description = 'Lead History4'
    ...
    crm = fields.Many2one('crm.lead','crm')
    attachment = fields.Many2many('ir.attachment', domain="[('res_model','=','crm.lead')]", string='첨부파일')
class LeadHistory5(models.Model):
    _name = "crm.lead.history5"
    _description = 'Lead History5'
